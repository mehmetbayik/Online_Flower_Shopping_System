...
FROM Employee e, Branch b
WHERE e.branch_id = b.branch_id AND e.birthdate < (SELECT birthdate FROM Employee WHERE SSN = b.manager_SSN);

-- 3) Find branch_id and name of branches which has female employees only

SELECT b.branch_id, b.bname
FROM Branch b, Employee e
WHERE b.branch_id = e.branch_id AND NOT EXISTS (SELECT * FROM Employee WHERE e.gender = 'M');
...
WHERE c.cust_id = a.cust_id AND c.cust_id = l.cust_id AND 
    (SELECT SUM(balance) FROM Account WHERE cust_id = c.cust_id) > 
    (SELECT SUM(amount) FROM Loan WHERE cust_id = c.cust_id);
...
WHERE b.branch_id = e.branch_id AND b.branch_id = a.branch_id AND a.balance > 1000000 HAVING COUNT(*) >= 50;
...
SELECT ename
FROM Employee
WHERE branch_id = (SELECT branch_id FROM Employee WHERE salary = (SELECT MAX(salary) FROM Employee));
